...
**Status update (same day, third pass): the Slack half of the P1 below is now live.** `_alert_slack()` posts to a `SLACK_WEBHOOK_URL` Incoming Webhook (no-op/log-only until that env var is set — see [`risk-register.md`](./risk-register.md) and [`operations-runbook.md`](./operations-runbook.md#4-monitoring--health-checks) for current wiring and the gap that remains: nothing external watches the process itself, only its own request/approval path). Structured per-request JSON logging (the other half of P1) is still open. Left the original analysis below as written — it's still the reasoning that led to the fix, not superseded by it.
