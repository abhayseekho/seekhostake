...
- [x] **Railway project webhook (Deployment Crashed / Deployment Oom Killed / Monitor & VolumeAlert Triggered → same `#seekhostake-alerts`) — done and verified 24 Sept.** Closes the specific gap the in-process Slack alerting above cannot: those three triggers all run inside the app process and die with it, but this fires from Railway's platform layer, independent of whether the app process is even alive. Same webhook URL, no new account. Confirmed via Railway's own "Test Webhook" (response 200) and by reading the formatted test message back from the channel. Configured in the dashboard (Project Settings → Webhooks) — not in source, so there's nothing to see in a diff; this line is the record of it. *(risk R-11, runbook §4)*
