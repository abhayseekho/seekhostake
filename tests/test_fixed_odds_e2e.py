...
def test_fixed_bet_locks_odds_and_settles_at_them(fixed_mode):
    ...
    # Alice bets ₹5,000 on Khuseel → locks (empty fixed book, cap ₹10k ⇒ 1 + 10000/5000 = 3.0x)
    ...
    assert pend["locked_odds"] == 3.0
    assert pend["locked_payout"] == 15000
    ...
    assert mine["locked_odds"] == 3.0 and mine["locked_payout"] == 15000

    # owner sees the fixed exposure: worst case is −cap (Khuseel wins → pay 15k on a 5k pool)
    house = admin.get("/api/state").json()["house"]
    assert house["fixed_exposure"] == -10000 and house["fixed_cap"] == 10000

    # run Khuseel 2–0 and settle → Alice is paid EXACTLY her locked 15,000 (not a pool split)
    ...
    assert alice_row["payout"] == 15000
    assert alice_row["net"] == 10000
    assert res["house_take"] == -10000  # house covered the locked payout, exactly the cap
